...
def prepare_acceptances(det: DetectorBase, aeffs: List[EffectiveAreaBase], energy_bins: List[Tuple], pars: Parameters, jd: float):
...
def get_flux_limits(
    ...
    aeffs: List[EffectiveAreaBase],
    gw: GW,
    parameters: Parameters,
    energy_bins: List[Tuple]
):
...
def plot_flux_limits(outfile: str, results: List[dict], sample_styles: dict):
    ...
    plt.close('all')
    for r in results:
        plt.plot(r['energy_bin'], [r['limit']]*2,
    ...
    for s, v in sample_styles.items():
        handles.append(matplotlib.lines.Line2D([], [], label=s, color=v['color'],
                       linestyle=v['style'] if 'style' in v else '-'))
